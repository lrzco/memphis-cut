...
// ⬇️ Field component EXTRAIT du composant parent pour préserver le focus
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full pl-10 pr-4 py-2.5 bg-surface-alt border rounded-xl text-ink placeholder-ink-subtle text-sm transition-all ${
            error ? 'border-red-400' : 'border-border hover:border-ink/20'
          }`}
          autoComplete={name}
          maxLength={isPassword ? 128 : 255}
        />
        {isPassword && onTogglePassword && (
          <button
            type="button"
            onClick={onTogglePassword}
            className="absolute right-3.5 top-1/2 -translate-y-1/2 text-ink-subtle hover:text-ink"
          >
...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const handleLogin = async () => {
    if (!checkRateLimit('login', 5, 60000)) {
      setGeneralError('Trop de tentatives. Réessayez dans 1 minute.');
      return;
    }
    ...
    try {
      ...
      setGeneralError('Erreur serveur.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (error) {
        setGeneralError(
          error.message.includes('already') ? 'Cet email est déjà utilisé' : error.message
        );
        return;
      }
      if (data.user?.identities?.length === 0) {
        setGeneralError('Cet email est déjà utilisé');
        return;
      }
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="px-6 py-6 space-y-3.5">
              ...
              {mode === 'register' && (
                <div className="p-3 rounded-xl bg-surface-alt border border-border-light">
                  ...
                  <div className="grid grid-cols-2 gap-1">
                    {[
                      { l: '8 caractères min', c: password.length >= 8 },
                      { l: 'Une majuscule', c: /[A-Z]/.test(password) },
                      { l: 'Une minuscule', c: /[a-z]/.test(password) },
                      { l: 'Un chiffre', c: /[0-9]/.test(password) },
                    ].map((r) => (
                      <div
                        key={r.l}
                        className={`text-xs flex items-center gap-1 ${
                          r.c ? 'text-green-600' : 'text-ink-subtle'
                        }`}
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${
                            r.c ? 'bg-green-500' : 'bg-ink-subtle/30'
                          }`}
                        />
                        {r.l}
                      </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full btn-primary py-3 text-sm flex items-center justify-center gap-2 disabled:opacity-50 mt-2"
              >
                {isLoading ? (
                  <>
              ...
              {mode === 'register' && (
                <p className="text-ink-subtle/60 text-xs text-center">
                  En créant un compte, vous acceptez nos conditions d'utilisation et politique de
                  confidentialité.
                </p>
              )}
            </form>
